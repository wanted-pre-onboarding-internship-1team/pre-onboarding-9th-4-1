...
export function usePagenation() {
  ...
  // let data = queryClient.getQueryData<MockResponse[]>(['mockList'], {}) || [];
  // data = data.filter(item => item.transaction_time.split(' ')[0] === TODAY);
  ...
  const onClickNextGroup = (page: number) => {
    setSearchParams({ page: String(currentGroup + 1) });
  };
