...
export function useMockList() {
  ...
  let pagedList = originList?.filter(item => item.transaction_time.split(' ')[0] === TODAY).sort((a, b) => a.id - b.id).slice(0, 50);

  if (page)
    pagedList = originList?.slice((page - 1) * 50, (page - 1) * 50 + 50);

  return pagedList.filter(item => item.transaction_time.split(' ')[0] === TODAY)
  .sort((a, b) => a.id - b.id)!; //data;
}
